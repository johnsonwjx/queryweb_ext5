...
public class PubFunc {
	 ...
	 * @throws Exception
	 */
	public static Hashtable<String, String> getQueryParams(HttpServletRequest request) throws Exception {
		...
		if (conditionXml != null && conditionXml.length() > 0) {
			...
			if (resultCondiXml != null && resultCondiXml.length() > 0) {
				conditionXml = URLDecoder.decode(resultCondiXml, "UTF-8");
			}
		}
		...
		String noSumFields = coverNull(paramMap.get("noSumFields")).toUpperCase();
		...
		if (autocolumninfo == null)
			...
			autocolumninfo = URLDecoder.decode(autocolumninfo, "UTF-8");
			autocolumninfo = URLDecoder.decode(autocolumninfo, "UTF-8");

		}

		String customInputCondi = paramMap.get("customInputCondi");
		customInputCondi = customInputCondi.replace("大于", " > ").replace("小于", " < ").replace("不等于", " != ").replace("等于", " = ");
		// 级次条件参数
		...
		paramHt.put("treeFilter", coverNull(paramMap.get("treeFilter")));
		...
		paramHt.put("noSumFields",noSumFields);
	...
	public static String unionXml(String desct, String source) throws DocumentException, UnsupportedEncodingException {
	...
	public static String parseReturnfieldlist(String returnfieldlist) throws Exception {
		if (returnfieldlist != null && returnfieldlist != "") {
			...
			returnfieldlist = returnfieldlist.substring(0, returnfieldlist.length() - 1);
		}
			...
			if ("##".equals(stc)) {
				...
				if (stcArr.length == 2 && index >= 0) {// 包含集团树，并且此节点有集团信息业务信息,形式为：集团code*业务code
					if (tc[0].indexOf("+") > 0) {
						...
						ConverterUtils.addNewTreeCondiElement(root, conNode, newStr, stcArr[1], opera, tc[0]);
						// ConverterUtils.addNewTreeCondiElement(root, conNode,
						// "fieldname", stcArr[1], opera, tc[0]);
					} else
						ConverterUtils.addNewTreeCondiElement(root, conNode, tc[0], stcArr[1], opera, tc[0]);

					ConverterUtils.addNewTreeCondiElement(root, conNode, "CORPCODE", stcArr[0], opera, "CORPCODE");
				} else if (stcArr.length == 1 && index >= 0) {// 包含集团树，并且此节点只有集团信息
					ConverterUtils.addNewTreeCondiElement(root, conNode, "CORPCODE", stcArr[0], "Llike", "CORPCODE");
				} else if (stcArr.length == 1 && index < 0) {// 不包含集团树的情况
					if (tc[0].indexOf("+") > 0) {
						String newStr = ((String) tc[0]).replaceAll("\\+", "加");
						ConverterUtils.addNewTreeCondiElement(root, conNode, newStr, stcArr[0], opera, newStr);
					} else
						ConverterUtils.addNewTreeCondiElement(root, conNode, tc[0], stcArr[0], opera, tc[0]);
				}
	 ...
	 * @return
	 */
	public static HashMap<String, String> getParamsMap(HttpServletRequest request) {
	...
	public static String coverNull(Object str) {
	...
	public static String coverMap2JsonStr(Map<String, String> map) {
	...
	public static String getMessageByRequest(HttpServletRequest request) {
		String msg = "";
		Enumeration<String> attrEnum = request.getAttributeNames();
		while (attrEnum.hasMoreElements()) {
			String key = attrEnum.nextElement();
			String value = PubFunc.coverNull(request.getAttribute(key));
			msg += key + "=|=" + value + "|=|";
		}
		return msg;
	}
	...
	public static boolean isAjax(HttpServletRequest request) {
		...
		return !StringUtils.nullOrBlank(requestType);
	}

	public static String getCodeCenterTableNameByAlias(String tableAlias) throws Exception {
		Hashtable<String, String> in = new Hashtable<String, String>();
		in.put("service", "codecenter.table.get");
		in.put("alias", tableAlias);
		Hashtable out = ServiceInvokerUtil.invoker(in);
		String xml = (String) out.get("XML");
		Document doc = DocumentHelper.parseText(xml);
		List lst = doc.selectNodes("/root/codetable/tablename");
	 ...
	 * @throws Exception
	 */
	public static String getUpdateSetStr(String updateInfo) throws Exception {
		String[] updateInfoArr = updateInfo.split(";");
		String updateStr = "";
		for (int i = 0; i < updateInfoArr.length; i++) {
			String[] infoArr = updateInfoArr[i].split("=");
			String updateField = infoArr[0];
			updateField = updateField.substring(updateField.indexOf("[") + 1, updateField.length() - 1);
			String updateValue = infoArr[1];
			if ("".equals(updateStr))
				updateStr = updateField + '=' + updateValue;
			else
				updateStr += "," + updateField + '=' + updateValue;
		}
		return updateStr;
	 ...
	 * 组织条件
	 */
	public static String getBuildCorpTreeCondi(String condi, String priviCondi) {
		if (!priviCondi.equals("undefined") && !"".equals(priviCondi) && priviCondi != null) {
			if (!condi.equals("undefined") && !"".equals(condi) && condi != null)
				return priviCondi + " and " + condi;
			else
				return priviCondi;
		} else {
			if (!condi.equals("undefined") && !"".equals(condi) && condi != null)
	...
	public static void putElementValue2Map(Map<String, String> resultMap, Element e) {
		resultMap.put("'" + e.getName() + "'", "'" + e.getText() + "'");
	}


	public static String getSysParam(String name, String corpId) throws Exception {
		Hashtable<String, String> ht = new Hashtable<String, String>();
		if (corpId.equals("##")) {
			corpId = "#";
		}
		ht.put("service", "parameter.getparams");
		ht.put("owner", corpId);
		ht.put("blur", "");
		ht.put("name", name.toUpperCase());
		ht = ServiceInvokerUtil.invoker(ht);
		String xml = (String) ht.get("XML");
		Document doc = DocumentHelper.parseText(xml);
		Element root = doc.getRootElement();
		String result = "";
		if (root != null && root.elements() != null && !root.elements().isEmpty()) {
			result = root.element("param").elementText("value");
		}
		return result;
	}
}
